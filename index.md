...
to [`utils::sessionInfo()`](https://rdrr.io/r/utils/sessionInfo.html),
but includes more information about packages, and where they were
installed from.

## Differences from `utils::sessionInfo()`

- Additional platform details: time zone, pandoc version, RStudio
  version, etc.
- Information about package sources, e.g. GitHub repo and hash for
  packages installed from GitHub.
- Highlight package installation problems, e.g. if the loaded and
  on-disk versions are different, if the MD5 checksum of the package DLL
  is wrong, etc.
- Highlight packages from unusual sources.
- Information about external software via
  [`external_info()`](https://sessioninfo.r-lib.org/reference/external_info.md).
- Information about the Python configuration if the reticulate package
  is loaded and configured.
- Information about package libraries.
- Compare two session info outputs with the
  [`session_diff()`](https://sessioninfo.r-lib.org/reference/session_diff.md)
  function.
...
![Screenshot of a terminal window demonstrating example output of the
sessioninfo
function.](https://raw.githubusercontent.com/r-lib/sessioninfo/main/man/figures/session-info2.svg)

### Copying to the clipboard

You can use the [`clipr`
package](https://cran.r-project.org/package=clipr) to copy the session
...
You can use the `to_file` argument of
...
[`session_diff()`](https://sessioninfo.r-lib.org/reference/session_diff.md)
can retrieve the session info from an URL or the clipboard and compare
it to the current session information:
...
Please note that the sessioninfo project is released with a [Contributor
Code of
Conduct](https://sessioninfo.r-lib.org/dev/CODE_OF_CONDUCT.html). By
